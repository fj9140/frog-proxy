...
func init() {
	if frogproxyCaErr != nil {
		panic("Error parsing bultin CA " + frogproxyCaErr.Error())
	}
	var err error
	if FrogproxyCa.Leaf, err = x509.ParseCertificate(FrogproxyCa.Certificate[0]); err != nil {
		panic("Error parsing bultin CA " + err.Error())
	}
}
...
var FrogproxyCa, frogproxyCaErr = tls.X509KeyPair(CA_CERT, CA_KEY)
